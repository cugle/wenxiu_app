...
(function($) {
    ...
    $.Autocompleter = function($elem, options) {
		...
		this.lastSelectedValue_1 = null;//todo
        /**
        ...
        $elem.focus(function(){
        	if (this.focusTimeout_) {
				clearInterval(this.focusTimeout_);
			}
			this.focusTimeout_ = setInterval(function() {
				self.activate();
			},self.options.delay);
        });
    ...
    $.Autocompleter.prototype.activateNow = function() {
        ...
        if ((value !== this.lastProcessedValue_ &&($.trim(value).length>0))&& value !== this.lastSelectedValue_1) {//todo
    ...
    $.Autocompleter.prototype.fetchRemoteData = function(filter, callback) {
        ...
        if (data) {
			...
			dataType=self.options.remoteDataType?
				(self.options.remoteDataType === 'json' ? 'json' : 'jsonp'):'text';
            ...
            console.log("in to post,url is "+this.makeUrl(filter));
            $.ajax({
				type:'POST',
    ...
    $.Autocompleter.prototype.selectItem = function($li) {
		...
		this.lastSelectedValue_1 = value;//todo
